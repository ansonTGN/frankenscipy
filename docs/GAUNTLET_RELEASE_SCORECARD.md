...
| `fsci-ndimage` label-indexed measurements performance | **complexity gap closed; mean constant cut** | `measurement_label_groups` moved shared label reductions from O(N·K) to O(N+K). Follow-up `mean(labels,index)` flat sum/count accumulation removes per-label bucket materialization for mean: another 1.44-1.77x internal win, 0 mismatches, but still 3.7-4.7x slower than SciPy's compiled C |
| `fsci-ndimage` label-indexed measurements correctness | guarded | byte-identical (`mism=0` vs old linear scan and previous bucketed route in `perf_label_stats`); full `cargo test -p fsci-ndimage --lib` 240 passed / 0 failed |
| `fsci-ndimage` lint/build gate | partial | `cargo check -p fsci-ndimage --all-targets`, touched-file rustfmt, `git diff --check`, and UBS passed; existing `fsci-interpolate`/`diff_geom` warnings and broad lint inventories remain outside this patch |
