...
- Baseline stage split from RCH `vmi1227854`: reduction `201.164 ms`, bidiagonal backend `80.808 ms`, U replay `56.941 ms`, V replay `105.913 ms`.
...
Reject. The candidate preserved the existing tolerance-level backend proof, but it was slower than the current tridiagonal backend in the same process (`0.709954x`). No source changes are retained.
