...
fn main() {
    ...
    for &n in &[3usize,6,12,30,60] {
        ...
        let a: Vec<Vec<f64>> = (0..n).map(|_| (0..n).map(|_| 0.1*r()).collect()).collect();
        ...
        for i in 0..n { for j in 0..n {
            ...
            let mut axat=0.0;
            for k in 0..n { let mut xat=0.0; for l in 0..n { xat+=x[k][l]*a[j][l]; } axat+=a[i][k]*xat; }
            mx=mx.max((axat - x[i][j] + q[i][j]).abs());
        }}
        println!("n={n}: max|AXA^T-X+Q| = {mx:.3e}"); worst=worst.max(mx);
    }
