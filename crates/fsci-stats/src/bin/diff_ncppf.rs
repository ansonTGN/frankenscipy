...
fn main() {
    ...
    for (df, nc) in &nct {
        ...
        for &q in &qs {
            println!("nct,{df},{nc},{q:.17e},{:.17e}", d.ppf(q));
        }
    }
    ...
    for (df, nc) in &ncx2 {
        ...
        for &q in &qs {
            println!("ncx2,{df},{nc},{q:.17e},{:.17e}", d.ppf(q));
            println!("ncx2i,{df},{nc},{q:.17e},{:.17e}", d.isf(q));
        }
    }
    let ncf: Vec<(f64, f64, f64)> = vec![(5.0, 12.0, 3.0), (4.0, 20.0, 2.0)];
    for (a, b, c) in &ncf {
        let d = NoncentralF::new(*a, *b, *c);
        for &q in &qs {
            println!("ncf,{a}_{b},{c},{q:.17e},{:.17e}", d.ppf(q));
        }
    }
}
