fn main() {
    ...
    println!(
        "mean.low/high = {}/{} (scipy: 3.334/7.666)",
        r.mean.low, r.mean.high
    );
    println!("var.statistic = {} (scipy: 11.7857)", r.variance.statistic);
    println!(
        "var.low/high = {}/{} (scipy: 4.337/30.551)",
        r.variance.low, r.variance.high
    );
    println!("std.statistic = {} (scipy: 3.3130)", r.std.statistic);
    println!(
        "std.low/high = {}/{} (scipy: 2.083/5.527)",
        r.std.low, r.std.high
    );
}
