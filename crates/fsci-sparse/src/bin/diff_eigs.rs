use fsci_sparse::{CooMatrix, Shape2D, eigs, EigsOptions};
...
fn main(){
  ...
  let a=CooMatrix::from_triplets(Shape2D::new(4,4),v,r,c,false).unwrap().to_csr().unwrap();
  match eigs(&a,2,EigsOptions::default()){
    Ok(res)=>{
      // (re, im) pairs, sorted by complex magnitude desc — scipy returns 3±4i.
      let mut e: Vec<(f64,f64)> = res.eigenvalues.iter().copied()
        .zip(res.eigenvalues_im.iter().copied()).collect();
