...
fn s(n:&str,v:f64){ println!("{n},{v:.17e}"); }
fn main(){
    // canberra/braycurtis with BOTH-zero components (0/0 edge)
    let u=[0.0,1.0,0.0,2.0]; let v=[0.0,3.0,0.0,2.0];
    ...
    let bu=[true,true,false,false]; let bv=[true,true,false,false]; // identical
    let fbu=[1.0,1.0,0.0,0.0]; s("jaccard_id", sp::jaccard(&fbu,&fbu));
    s("dice_id", sp::dice(&bu,&bv));
    s("yule_id", sp::yule(&bu,&bv));
    let af=[false,false,false,false]; // all false both
    let faf=[0.0,0.0,0.0,0.0]; s("jaccard_allfalse", sp::jaccard(&faf,&faf));
    ...
    let y1=[true,true,true,false]; let y2=[true,true,false,false];
    s("yule_nodiscord", sp::yule(&y1,&y2));
    // jaccard float (nonzero/zero pattern)
    let fu=[1.0,0.0,2.0,0.0]; let fv=[0.0,0.0,3.0,0.0];
