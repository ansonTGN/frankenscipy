...
use fsci_signal::{BaCoeffs, FilterType, SignalError, bessel, butter, cheby1, cheby2, ellip};
...
fn main() {
    ...
    for order in [2usize, 3, 4, 5] {
        ...
        dump(
            &format!("cheby1_lp_{order}"),
            cheby1(order, 1.0, &lp, Lowpass),
        );
        dump(
            &format!("cheby1_bp_{order}"),
            cheby1(order, 1.0, &bp, Bandpass),
        );
        dump(
            &format!("cheby2_lp_{order}"),
            cheby2(order, 40.0, &lp, Lowpass),
        );
        dump(
            &format!("cheby2_hp_{order}"),
            cheby2(order, 40.0, &hp, Highpass),
        );
        dump(
            &format!("ellip_lp_{order}"),
            ellip(order, 1.0, 40.0, &lp, Lowpass),
        );
        dump(
            &format!("ellip_bp_{order}"),
            ellip(order, 1.0, 40.0, &bp, Bandpass),
        );
