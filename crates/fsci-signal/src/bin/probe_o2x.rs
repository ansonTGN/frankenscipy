...
fn main(){
    ...
    let s2x=ShortTimeFft::new(hann(8),3,100.0).unwrap().with_scale_to(StftScaling::Psd).with_fft_mode(StftFftMode::Onesided2X);
    let s=s2x.stft(&x).unwrap();
    print!("O2X"); for row in &s { for c in row { print!(" {:.12e} {:.12e}", c.0, c.1); } } println!();
    let xr=s2x.istft(&s,0,None).unwrap();
    let m=xr.len().min(n); let mut e=0.0f64; for i in 0..m { e=e.max((xr[i]-x[i]).abs()); }
