...
fn main() {
    ...
    for &(img, size) in &[([64usize, 64usize], 3usize), ([128, 128], 5), ([200, 200], 7)] {
        let input = arr(&img, 7);
    ...
    for &(img, size) in &[([256usize, 256usize], 7usize), ([400, 400], 9)] {
        ...
        let mut acc = 0.0;
        for _ in 0..reps {
            let out = generic_filter(black_box(&input), nvar, size, BoundaryMode::Reflect, 0.0).unwrap();
            acc += out.data[out.data.len() / 2];
        }
        println!("img={img:?} size={size}  {:>10.3?}/call (acc={acc:.6})", t0.elapsed() / reps);
    }
}
